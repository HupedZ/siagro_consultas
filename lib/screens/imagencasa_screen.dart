...
class FullScreenImageCasa extends StatelessWidget {
                ...
                child: imageCasa1 != null && imageCasa1!.isNotEmpty
                    ? Column(children: [
                        InteractiveViewer(
                          ...
                          maxScale: 5.0,
                          child:
                              Image.network(imageCasa1!, fit: BoxFit.contain),
                        ),
                        IconButton(
                          onPressed: () async {
                          ...
                          if (result != null) {
                            try {
                              ...
                              if (imageUrl != null) {
                                try {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content:
                                          Text('¡Error al subir la imagen! $e'),
                                      backgroundColor: Colors
                                          .red, // Puedes personalizar el color de fondo.
                                    ),
                                  );
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(
                                      '¡Ninguna imagen fue seleccionada! $e'),
                                  backgroundColor: Colors
                                      .red, // Puedes personalizar el color de fondo.
                                ),
                              );
  ...
  Future<String?> uploadImage(XFile? pickedFile) async {
    final url = Uri.parse(
        'https://api.cloudinary.com/v1_1/dxtncec6a/image/upload?upload_preset=pgqwkshy');
